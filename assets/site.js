...
(function () {
  ...
  if (nav) nav.innerHTML =
    ...
    '<div id="mobile-menu" class="m-menu" hidden>' +
      ...
      '<button aria-expanded="false" data-acc="m-min">Ministries<span class="material-symbols-outlined">expand_more</span></button><div class="m-sub" id="m-min" hidden>' +
        clubs.map(function (c) { return '<a href="' + c.href + '">' + c.name + '</a><div class="m-sub">' + c.sub.map(function (s) { return '<a href="' + s[1] + '">' + s[0] + '</a>'; }).join('') + '</div>'; }).join('') + '</div>' +
  ...
  if (foot) foot.innerHTML =
    ...
    '<div><h4>Ministries</h4><ul>' + clubs.map(function (c) { return '<li><a href="' + c.href + '">' + c.name + '</a></li>'; }).join('') + '<li><a href="' + u('ministries/pathfinders/bible-experience.html') + '">Bible Experience (PBE)</a></li><li><a href="' + u('brief.html#TLT') + '">Teen Leadership Training</a></li></ul></div>' +
